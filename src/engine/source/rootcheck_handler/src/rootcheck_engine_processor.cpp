#include "rootcheck_handler/rootcheck_engine_processor.hpp"
#include <base/logging.hpp>
#include <base/json.hpp> // For creating the base::Event and parsing JSON payloads
#include <base/utils/timeUtils.hpp> // For base::utils::TimePointToString_ISO8601_UTC

#include <cstring> // For strncmp, strchr, etc.
#include <sstream> // For string manipulation, if needed for legacy parsing
...
// Implementation of RootcheckEngineProcessor methods
RootcheckEngineProcessor::RootcheckEngineProcessor(std::shared_ptr<WazuhDbRootcheckInterface> db_interface)
    : m_db_interface(std::move(db_interface)) {
    if (!m_db_interface) {
        throw std::runtime_error("RootcheckEngineProcessor: WazuhDbRootcheckInterface is null.");
    }
}

ParsedRootcheckData RootcheckEngineProcessor::parseRootcheckPayload(const std::string& payload, const std::string& agent_id, const std::string& location) {
    ParsedRootcheckData data;
    // Rootcheck messages are typically plain text, but can sometimes be JSON-like if an agent sends custom JSON.
    // A simple approach for PoC:
    // Example legacy: "Rootcheck: System audit event: Anomaly detected in file '/etc/test'. Details: High deviation from baseline."
    // Example newer (could be JSON in actual_log_payload): {"title":"Host-based anomaly detection event", "file":"/root/test", "pci_dss":"11.5", ...}

    LOG_DEBUG("RootcheckEngineProcessor: Parsing payload for agent '{}', location '{}': {}", agent_id, location, payload);

    // Attempt to parse as JSON first (if some agents send structured rootcheck)
    try {
        json::Json rootcheck_json(payload.c_str());
        if (!rootcheck_json.isNull() && rootcheck_json.isObjectType()) {
            data.message_title = rootcheck_json.getString("/title").value_or(rootcheck_json.getString("/message").value_or("Rootcheck finding"));
            data.pci_dss = rootcheck_json.getString("/pci_dss").value_or("");
            data.cis = rootcheck_json.getString("/cis").value_or("");
            data.file_path = rootcheck_json.getString("/file").value_or(rootcheck_json.getString("/path").value_or(""));
            data.event_details = rootcheck_json.getString("/details").value_or(payload); // Fallback to full payload
            data.level = rootcheck_json.getInt("/level").value_or(5); // Default level for JSON
            LOG_DEBUG("RootcheckEngineProcessor: Parsed rootcheck payload as JSON for agent '{}'", agent_id);
            return data;
        }
    } catch (const std::exception& e) {
        LOG_DEBUG("RootcheckEngineProcessor: Payload for agent '{}' is not JSON. Attempting legacy parse. Error: {}", agent_id, e.what());
    }

    // Legacy/Plain text parsing (very basic for PoC)
    // Example: "Rootcheck: System audit event." -> location is the key
    // Example: "File integrity change detected on /etc/passwd."
    // Example: "Ending Rootcheck scan."
    
    if (payload.rfind("Rootcheck: ", 0) == 0) {
        data.message_title = payload.substr(strlen("Rootcheck: "));
        trim_string_inplace(data.message_title);
        // Try to extract a path if mentioned
        size_t path_pos = payload.find("on /");
        if (path_pos == std::string::npos) path_pos = payload.find("file /");
        if (path_pos != std::string::npos) {
            // find the space after path starts
            size_t path_start = payload.find('/', path_pos);
            if(path_start != std::string::npos){
                size_t path_end = payload.find_first_of(" .,;:!?)", path_start);
                if (path_end == std::string::npos) path_end = payload.length();
                data.file_path = payload.substr(path_start, path_end - path_start);
                trim_string_inplace(data.file_path);
            }
        }
        // Basic level inference (highly simplified)
        if (payload.find("Anomaly detected") != std::string::npos || payload.find("malware") != std::string::npos) {
            data.level = 7;
        } else {
            data.level = 5;
        }
        data.event_details = payload; // Store full payload as details for now
    } else if (payload.rfind("Ending Rootcheck scan.", 0) == 0 || payload.rfind("Starting Rootcheck scan.", 0) == 0) {
        data.message_title = payload;
        data.level = 3; // Informational
        data.event_details = payload;
    } else {
        data.message_title = "Generic Rootcheck finding";
        data.event_details = payload;
        data.level = 5; // Default
    }
    
    if (data.file_path.empty() && !location.empty() && location != "syscheck") { // "syscheck" is often the location for rootcheck events
        data.file_path = location; // Use 'location' from CleanedMessageData if no path in payload
    }


    LOG_DEBUG("RootcheckEngineProcessor: Parsed rootcheck payload (legacy) for agent '{}', title '{}'", agent_id, data.message_title);
    return data;
}

base::Event RootcheckEngineProcessor::createRootcheckAlertEvent(
    const input_adapters::CleanedMessageData& original_msg_meta,
    const ParsedRootcheckData& parsed_data) 
{
    base::Event rootcheck_alert_event = std::make_shared<json::Json>(json::JsonType::Object);

    // Standard fields
    rootcheck_alert_event->setString(original_msg_meta.agent_id, "/agent/id");
    if (!original_msg_meta.agent_name.empty()) {
         rootcheck_alert_event->setString(original_msg_meta.agent_name, "/agent/name");
    }
     if (!original_msg_meta.agent_ip.empty()) {
         rootcheck_alert_event->setString(original_msg_meta.agent_ip, "/agent/ip");
    }
    rootcheck_alert_event->setString(original_msg_meta.raw_full_message, "/input/raw_rootcheck_message");
    rootcheck_alert_event->setString("wazuh_queue", "/input/adapter_name"); // Identify source as wazuh_queue


    // Rootcheck specific fields
    json::Json rootcheck_section = json::Json::object();
    rootcheck_section.setString(parsed_data.message_title, "/title");
    if (!parsed_data.pci_dss.empty()) rootcheck_section.setString(parsed_data.pci_dss, "/pci_dss");
    if (!parsed_data.cis.empty()) rootcheck_section.setString(parsed_data.cis, "/cis");
    if (!parsed_data.file_path.empty()) rootcheck_section.setString(parsed_data.file_path, "/path");
    rootcheck_section.setString(parsed_data.event_details, "/details");
    
    rootcheck_alert_event->set("/rootcheck", rootcheck_section);

    // Rule information (placeholders, actual rule matching is separate)
    // Rootcheck alerts often have specific rule IDs based on the check.
    // This part would be more dynamic in a full implementation.
    int rule_id = 550; // Default rootcheck rule ID
    if (parsed_data.message_title.find("System audit event") != std::string::npos) rule_id = 550;
    else if (parsed_data.message_title.find("File integrity change") != std::string::npos) rule_id = 554; // Or a FIM rule if it's a file event
    
    rootcheck_alert_event->setInt(rule_id, "/rule/id"); 
    rootcheck_alert_event->setInt(parsed_data.level, "/rule/level"); 
    rootcheck_alert_event->setString(parsed_data.message_title, "/rule/description"); // Use title as description for now
    
    json::Json groups = json::Json::array();
    groups.add("rootcheck");
    groups.add("ossec"); // common group
    rootcheck_alert_event->set("/rule/groups", groups);

    // Timestamp
    rootcheck_alert_event->setString(base::utils::TimePointToString_ISO8601_UTC(std::chrono::system_clock::now()), "/timestamp");

    return rootcheck_alert_event;
}


std::optional<base::Event> RootcheckEngineProcessor::processRootcheckMessage(const input_adapters::CleanedMessageData& msg)
{
    LOG_DEBUG("RootcheckEngineProcessor: Processing message for agent: {}, location: {}, payload: {}", 
              msg.agent_id, msg.location, msg.actual_log_payload);

    if (msg.agent_id.empty() || msg.actual_log_payload.empty()) {
        LOG_ERROR("RootcheckEngineProcessor: Agent ID or payload is empty. Cannot process Rootcheck message.");
        return std::nullopt;
    }

    // 1. Parse the Rootcheck payload
    ParsedRootcheckData parsed_rootcheck_data = parseRootcheckPayload(msg.actual_log_payload, msg.agent_id, msg.location);

    // Handle special rootcheck messages (scan start/end)
    if (parsed_rootcheck_data.message_title == "Starting Rootcheck scan." || 
        parsed_rootcheck_data.message_title == "Starting syscheck scan." ) { // Syscheck often includes rootcheck
        if (m_db_interface->connect()) {
            m_db_interface->clearAgentRootcheck(msg.agent_id);
            m_db_interface->updateLastScan(msg.agent_id, std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
            m_db_interface->disconnect();
        } else {
            LOG_ERROR("RootcheckEngineProcessor: Failed to connect to wazuh-db for rootcheck start operations for agent {}.", msg.agent_id);
        }
        // Typically, scan start messages don't generate alerts themselves, but log them.
        LOG_INFO("RootcheckEngineProcessor: {} Agent: {}", parsed_rootcheck_data.message_title, msg.agent_id);
        return std::nullopt; 
    }
    if (parsed_rootcheck_data.message_title == "Ending Rootcheck scan." ||
        parsed_rootcheck_data.message_title == "Ending syscheck scan.") {
        if (m_db_interface->connect()) {
            m_db_interface->updateLastScan(msg.agent_id, std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
            m_db_interface->disconnect();
        } else {
            LOG_ERROR("RootcheckEngineProcessor: Failed to connect to wazuh-db for rootcheck end operations for agent {}.", msg.agent_id);
        }
        LOG_INFO("RootcheckEngineProcessor: {} Agent: {}", parsed_rootcheck_data.message_title, msg.agent_id);
        // Some systems might generate a summary alert for scan end. For PoC, just log.
        return std::nullopt; 
    }

    // 2. Store the rootcheck finding in wazuh-db
    // The format of rootcheckData sent to DB needs to match what wazuh-db expects.
    // For PoC, we assume the `actual_log_payload` or a derivative of `parsed_rootcheck_data` is suitable.
    // A more robust implementation would format `parsed_rootcheck_data` into the precise string/JSON.
    // For now, we'll use the actual_log_payload as a placeholder for what gets stored.
    std::string data_to_store = msg.actual_log_payload; // Placeholder for formatted data
    
    if (m_db_interface->connect()) {
        if (!m_db_interface->storeRootcheckEntry(msg.agent_id, data_to_store)) {
            LOG_ERROR("RootcheckEngineProcessor: Failed to store rootcheck entry in wazuh-db for agent: {}. Data: {}", msg.agent_id, data_to_store);
            // Decide if an alert should still be generated if DB storage fails. For PoC, we will.
        }
        m_db_interface->disconnect();
    } else {
        LOG_ERROR("RootcheckEngineProcessor: Failed to connect to wazuh-db. Cannot store rootcheck entry for agent: {}", msg.agent_id);
        // Alert generation might still proceed based on policy.
    }

    // 3. Construct and return a base::Event for the rootcheck finding.
    // All rootcheck findings (other than scan start/end) are typically considered alerts.
    LOG_INFO("RootcheckEngineProcessor: Rootcheck finding for agent: {}. Title: {}", msg.agent_id, parsed_rootcheck_data.message_title);
    return createRootcheckAlertEvent(msg, parsed_rootcheck_data);
}
